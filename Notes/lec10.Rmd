...
and so on.
- If `g` is called within `f`, then the enclosing environment
of `g` is the environment of `f`.
...
*  The following code chunk is typed into the R Console.
    ...
    - What is the enclosing environment of this
    call to `g()`?
    - What search order does R use to find the 
    value of `x` when it is needed in this call to `g()`?
```{r}
x <- 1
f <- function(y) {
  g(y)
