...
`magrittr` package, which is loaded when we load the tidyverse.
...
to take an argument `na.rm`. Set this argument 
to `TRUE` by default.
...
* Example: In the following, `seq_along(nreps)` is the "index set".

\scriptsize

```{r}
n <- 10; nreps <- 100; x <- vector(mode="numeric",length=nreps)
for(i in seq_along(nreps)) {
  x[i] <- mean(rnorm(n))
}
summary(x)
print(i)
...
Hint: If `tt` is a tibble, `ncol(tt)` is 
the number of columns, and `seq_along(ncol(tt))` is 
an appropriate index set.


## `for` loop index set

- Index sets of the form `seq_along(n)` are most common, but 
can be almost any vector.
...
* In which environment do the following functions find the object `y`?
